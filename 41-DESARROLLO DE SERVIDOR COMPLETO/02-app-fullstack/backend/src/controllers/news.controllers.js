import { getAllNews, getNew, createNew, updateNew, deleteNew } from "../services/news.services.js";
...
export async function updateNewCtr(req, res) {
  try {
    ...
    if(!newToModified){
      ...
      const newUpdated = await updateNew({ _id: id },
                {
                  $set: {
                    title,
                    body,
